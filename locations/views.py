...
def countries_in_continent(request, continent):
    try:
        ...
        logger.write_to_console(str(ex), 'countries in continents')
...
def import_data_to_database(request):
    ...
    try:
        if request.method == 'POST':
            ...
            full_filename = os.path.join(settings.MEDIA_ROOT, excel_file.name)
            ...
            if 'states' in request.POST:
                for column in data_frame:
                    try:
                        ...
                        logger.write_to_console(str(ex), 'Import')
